...
-- function_name
function M.lua(args)
	local name = args.name or string.format("%s.%s", args.module_name, args.function_name)
	local command_args = string.format('require("%s").%s()', args.module_name, args.function_name)
	return M.command({
		name = name,
		command = "lua",
